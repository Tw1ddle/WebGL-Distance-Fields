...
Main.prototype = {
	onWindowLoaded: function() {
		...
		window.addEventListener("keydown",function(event1) {
			var keycode;
			if(event1.keyCode == null) keycode = event1.charCode; else keycode = event1.keyCode;
			if(keycode == 8 || keycode == 46) _g.removeCharacter();
		},true);
	...
	,generateDistanceFieldForString: function(s) {
		if(!this.characterMap.exists(s)) {
			...
			this.generateDistanceField(data.canvas,data.metrics,s);
		}
	}
	,generateDistanceField: function(element,metrics,id) {
		var texture = new THREE.Texture(element,THREE.UVMapping);
		texture.needsUpdate = true;
		var target = this.sdfMaker.transformTexture(texture,true);
		var geometry = new THREE.PlaneGeometry(target.width,target.height);
		var aaMaterial = new THREE.ShaderMaterial({ vertexShader : sdf_shaders_EDT_$DISPLAY_$AA.vertexShader, fragmentShader : sdf_shaders_EDT_$DISPLAY_$AA.fragmentShader, uniforms : THREE.UniformsUtils.clone(sdf_shaders_EDT_$DISPLAY_$AA.uniforms)});
		...
		aaMaterial.uniforms.texLevels.value = this.sdfMaker.texLevels;
		aaMaterial.uniforms.threshold.value = 0.0;
		this.characterMap.set(id,new Character(geometry,aaMaterial,target.width,target.height,metrics));
	}
