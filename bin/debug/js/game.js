...
var Main = function() {
	...
	this.displayShaders = ["AA","OVERLAY","RGB","PASSTHROUGH"];
...
Main.prototype = {
	onWindowLoaded: function() {
		...
		if(glSupported != 0) {
			...
			switch(glSupported) {
			case 2:
				unsupportedInfo.innerHTML = "Your browser does not support WebGL. Click <a href=\"" + "https://github.com/Tw1ddle/WebGLDistanceFields" + "\" target=\"_blank\">here for project info</a> instead.";
				break;
			case 1:
				unsupportedInfo.innerHTML = "Your browser supports WebGL, but the feature appears to be disabled. Click <a href=\"" + "https://github.com/Tw1ddle/WebGLDistanceFields" + "\" target=\"_blank\">here for project info</a> instead.";
				break;
			default:
				unsupportedInfo.innerHTML = "Could not detect WebGL support. Click <a href=\"" + "https://github.com/Tw1ddle/WebGLDistanceFields" + "\" target=\"_blank\">here for project info</a> instead.";
			}
		...
		if(ext == null) {
			...
			missingExtensionInfo.innerHTML = "Missing required WebGL extension: " + extDerivatives + " Click <a href=\"" + "https://github.com/Tw1ddle/WebGLDistanceFields" + "\" target=\"_blank\">here for project info</a> instead.";
	...
	,loadTexture: function(url) {
		...
		var texture1 = THREE.ImageUtils.loadTexture(url,THREE.UVMapping,function(texture) {
			_g.sdfMaker.transformTexture(texture,"",false,false);
		});
	}
	,loadCanvas: function(element,id) {
		var texture = new THREE.Texture(element,THREE.UVMapping);
		texture.needsUpdate = true;
		this.sdfMaker.transformTexture(texture,id,true,false);
	}
	...
	,onDisplayShaderChanged: function(id) {
		...
		case "OVERLAY":
			var _g3 = 0;
			var _g12 = this.overlayMaterials;
		...
		case "PASSTHROUGH":
			var _g4 = 0;
			var _g13 = this.passthroughMaterials;
...
var SDFMaker = function(renderer) {
	...
	this.filterMaterial = new THREE.ShaderMaterial({ vertexShader : shaders_HermiteFilter.vertexShader, fragmentShader : shaders_HermiteFilter.fragmentShader, uniforms : shaders_HermiteFilter.uniforms});
...
SDFMaker.prototype = {
	transformTexture: function(texture,id,blurInput,bicubicFilterInput) {
		if(bicubicFilterInput == null) bicubicFilterInput = true;
		if(blurInput == null) blurInput = false;
		...
		if(bicubicFilterInput) {
			this.scene.overrideMaterial = this.filterMaterial;
			this.filterMaterial.uniforms.tDiffuse.value = texture;
			this.filterMaterial.uniforms.texw.value = width;
			this.filterMaterial.uniforms.texh.value = height;
			...
			this.copyMaterial.uniforms.tDiffuse.value = this.pong;
			this.renderer.render(this.scene,this.camera);
		}
		if(blurInput) {
		...
		if(blurInput || bicubicFilterInput) this.seedMaterial.uniforms.tDiffuse.value = this.pong; else this.seedMaterial.uniforms.tDiffuse.value = texture;
...
var shaders_HermiteFilter = function() { };
shaders_HermiteFilter.__name__ = true;
...
Main.REPO_URL = "https://github.com/Tw1ddle/WebGLDistanceFields";
...
shaders_EDT_$DISPLAY_$OVERLAY.vertexShader = "varying vec2 vUv;\r\nvarying float oneu;\r\nvarying float onev;\r\n\r\nuniform float texw;\r\nuniform float texh;\r\n\r\nvoid main()\r\n{\r\n\tvUv = uv;\r\n\t\r\n\t// Save divisions in some of the fragment shaders\r\n\toneu = 1.0 / texw;\r\n\tonev = 1.0 / texh;\r\n\t\r\n\tgl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);\r\n}";
shaders_EDT_$DISPLAY_$OVERLAY.fragmentShader = "// Distance map contour texturing by Stefan Gustavson, 2011.\r\n// A reimplementation of Greens method, with a 16-bit 8:8 distance map and explicit bilinear interpolation.\r\n// This code is in the public domain.\r\n\r\nvarying vec2 vUv;\r\nvarying float oneu;\r\nvarying float onev;\r\n\r\nuniform sampler2D tDiffuse;\r\nuniform float texw;\r\nuniform float texh;\r\nuniform float texLevels;\r\nuniform float threshold;\r\n\r\n// Replacement for RSLs filterstep(), with fwidth() done right.\r\n// threshold is constant, value is smoothly varying\r\nfloat aastep(float threshold, float value)\r\n{\r\n\tfloat afwidth = 0.7 * length(vec2(dFdx(value), dFdy(value)));\r\n\treturn smoothstep(threshold - afwidth, threshold + afwidth, value); // GLSLs fwidth(value) is abs(dFdx(value)) + abs(dFdy(value))\r\n}\r\n\r\n// Helper functions to remap unsigned normalized floats [0.0, 1.0] coming from an integer texture to the range we need [-1, 1].\r\n// The transformations are very specifically designed to map integer texel values exactly to pixel centers, and vice versa.\r\nvec2 remap(vec2 floatdata)\r\n{\r\n\treturn floatdata * (texLevels - 1.0) / texLevels * 2.0 - 1.0;\r\n}\r\n\r\nvoid main()\r\n{\r\n\t// Scale texcoords to range ([0, texw], [0, texh])\r\n\tvec2 uv = vUv * vec2(texw, texh);\r\n\t\r\n\t// Compute texel-local (u,v) coordinates for the four closest texels\r\n\tvec2 uv00 = floor(uv - vec2(0.5)); // Lower left corner of lower left texel\r\n\tvec2 uvlerp = uv - uv00 - vec2(0.5); // Texel-local lerp blends [0,1]\r\n\t\r\n\t// Center st00 on lower left texel and rescale to [0,1] for texture lookup\r\n\tvec2 st00 = (uv00 + vec2(0.5)) * vec2(oneu, onev);\r\n\t\r\n\t// Compute distance value from four closest 8-bit RGBA texels\r\n\tvec4 T00 = texture2D(tDiffuse, st00);\r\n\tvec4 T10 = texture2D(tDiffuse, st00 + vec2(oneu, 0.0));\r\n\tvec4 T01 = texture2D(tDiffuse, st00 + vec2(0.0, onev));\r\n\tvec4 T11 = texture2D(tDiffuse, st00 + vec2(oneu, onev));\r\n\tfloat D00 = length(remap(T00.rg)) + (T00.b - 0.5) / texw;\r\n\tfloat D10 = length(remap(T10.rg)) + (T10.b - 0.5) / texw;\r\n\tfloat D01 = length(remap(T01.rg)) + (T01.b - 0.5) / texw;\r\n\tfloat D11 = length(remap(T11.rg)) + (T11.b - 0.5) / texw;\r\n\t\r\n\t// Interpolate along v\r\n\tvec2 D0_1 = mix(vec2(D00, D10), vec2(D01, D11), uvlerp.y);\r\n\t\r\n\t// Interpolate along u\r\n\tfloat D = mix(D0_1.x, D0_1.y, uvlerp.x);\r\n\t\r\n\tfloat g = aastep(threshold, D);\r\n\t\r\n\t// Retrieve the B channel to get the original grayscale image\r\n\tfloat c = texture2D(tDiffuse, vUv).b;\r\n\t\r\n\t// Final fragment color\r\n\tgl_FragColor = vec4(vec3(g, c, g), 1.0);\r\n}";
...
shaders_EDT_$DISPLAY_$RGB.vertexShader = "varying vec2 vUv;\r\nvarying float oneu;\r\nvarying float onev;\r\n\r\nuniform float texw;\r\nuniform float texh;\r\n\r\nvoid main()\r\n{\r\n\tvUv = uv;\r\n\t\r\n\t// Save divisions in some of the fragment shaders\r\n\toneu = 1.0 / texw;\r\n\tonev = 1.0 / texh;\r\n\t\r\n\tgl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);\r\n}";
