...
Main.prototype = {
	onWindowLoaded: function() {
		...
		var width = this.sceneWidth * this.renderer.getPixelRatio();
		var height = this.sceneHeight * this.renderer.getPixelRatio();
		this.scene = new THREE.Scene();
		this.camera = new THREE.OrthographicCamera(width / -2,width / 2,height / 2,height / -2,1,1000);
		this.camera.position.z = 80;
		this.signal_windowResized.add(function() {
			var width1 = _g.sceneWidth * _g.renderer.getPixelRatio();
			var height1 = _g.sceneHeight * _g.renderer.getPixelRatio();
			_g.renderer.setSize(width1,height1);
			_g.camera.left = width1 / -2;
			_g.camera.top = height1 / 2;
			_g.camera.right = width1 / 2;
			_g.camera.bottom = height1 / -2;
		...
		this.sdfMaker.signal_textureLoaded.add(function(id,target) {
			...
			var aaMaterial = new THREE.ShaderMaterial({ vertexShader : sdf_shaders_EDT_$DISPLAY_$AA.vertexShader, fragmentShader : sdf_shaders_EDT_$DISPLAY_$AA.fragmentShader, uniforms : sdf_shaders_EDT_$DISPLAY_$AA.uniforms});
			...
			var grayscaleMaterial = new THREE.ShaderMaterial({ vertexShader : sdf_shaders_EDT_$DISPLAY_$GRAYSCALE.vertexShader, fragmentShader : sdf_shaders_EDT_$DISPLAY_$GRAYSCALE.fragmentShader, uniforms : sdf_shaders_EDT_$DISPLAY_$GRAYSCALE.uniforms});
			...
			var rgbMaterial = new THREE.ShaderMaterial({ vertexShader : sdf_shaders_EDT_$DISPLAY_$RGB.vertexShader, fragmentShader : sdf_shaders_EDT_$DISPLAY_$RGB.fragmentShader, uniforms : sdf_shaders_EDT_$DISPLAY_$RGB.uniforms});
			...
			var overlayMaterial = new THREE.ShaderMaterial({ vertexShader : sdf_shaders_EDT_$DISPLAY_$OVERLAY.vertexShader, fragmentShader : sdf_shaders_EDT_$DISPLAY_$OVERLAY.fragmentShader, uniforms : sdf_shaders_EDT_$DISPLAY_$OVERLAY.uniforms});
			...
			var copyMaterial = new THREE.ShaderMaterial({ vertexShader : sdf_shaders_Copy.vertexShader, fragmentShader : sdf_shaders_Copy.fragmentShader, uniforms : sdf_shaders_Copy.uniforms});
		...
		window.addEventListener("dragover",function(event2) {
			event2.preventDefault();
			event2.stopPropagation();
		},false);
		window.addEventListener("drop",function(event3) {
			var dt = event3.dataTransfer;
			var files = dt.files;
			var i = 0;
			while(i < files.length) {
				var f = [files.item(i)];
				if(!f[0].type.match("image.*")) break;
				...
				reader.onload = (function(f) {
					return function(e) {
						...
						_g.sdfMaker.transformTexture(texture,f[0].name,false);
						_g.sceneWidth = image.width;
						_g.sceneHeight = image.height;
						_g.signal_windowResized.dispatch();
					};
				})(f);
	...
	,setupGUI: function() {
		...
		dat_ShaderGUI.generate(this.shaderGUI,"AA",sdf_shaders_EDT_$DISPLAY_$AA.uniforms);
		dat_ShaderGUI.generate(this.shaderGUI,"OVERLAY",sdf_shaders_EDT_$DISPLAY_$OVERLAY.uniforms);
		dat_ShaderGUI.generate(this.shaderGUI,"GRAYSCALE",sdf_shaders_EDT_$DISPLAY_$GRAYSCALE.uniforms);
		dat_ShaderGUI.generate(this.shaderGUI,"RGB",sdf_shaders_EDT_$DISPLAY_$RGB.uniforms);
		dat_ShaderGUI.generate(this.shaderGUI,"PASSTHROUGH",sdf_shaders_Copy.uniforms);
		dat_ShaderGUI.generate(this.shaderGUI,"SEED",sdf_shaders_EDT_$SEED.uniforms);
		dat_ShaderGUI.generate(this.shaderGUI,"FLOOD",sdf_shaders_EDT_$FLOOD.uniforms);
	}
